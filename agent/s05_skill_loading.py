...
# Anthropic这家模型服务 BASE基础 URL接口地址
# URL 是 Uniform Resource Locator 的缩写。
# 中文通常翻译为：统一资源定位符
# Anthropic authentication token
# Anthropic 的身份认证令牌或者Anthropic API 访问令牌
...
@dataclass
class SkillManifest:  # 专门保存某个skill的基本信息 manifest翻译为清单
    name: str         # 技能名
    description: str  # 技能描述
    path: Path        # 这个技能文件所在路径

@dataclass
class SkillDocument:
    manifest: SkillManifest # 这个 skill 的元信息
    body: str               # 这个 skill 的正文内容
...
class SkillRegistry:
    ...
    def _load_all(self) -> None:
        if not self.skills_dir.exists():
            return
        for path in sorted(
            self.skills_dir.rglob("SKILL.md") # rglob递归搜索目录及其所有子目录中的文件
        ):
            meta, body = self._parse_frontmatter(path.read_text(encoding="utf-8"))
            name = meta.get("name", path.parent.name) # 优先frontmatter里的name, 如果没有就用父目录名当技能名
